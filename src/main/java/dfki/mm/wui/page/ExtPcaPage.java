...
import dfki.mm.tracks.extension.PCAExtension;
...
public class ExtPcaPage extends PageTemplate implements MessageListener {

    private static final Logger log = LoggerFactory.getLogger(ExtPcaPage.class);
    ...
    public ExtPcaPage(String filename, String pageName, String address) {
...
//        docMain.getElementById(MyApiField.field.name()).attr("multiple", false);
        ...
        docMain.getElementById(HEADER_LIST).text("Existing PCA");
        docMain.getElementById(HEADER_CREATE).text("Add PCA:");
        docMain.getElementById("form-request").remove();
        docMain.getElementById("form-submit")
                .attr("name", "request")
                .attr("value", "create");
        docMain.getElementById("form-buttons").appendElement(HTML.Tag.INPUT.toString())
                .attr("type", "submit")
                .attr("name", "request")
                .attr("value", "preview");
        docMain.getElementById("form-buttons").appendElement(HTML.Tag.BR.toString());
...
//        }
    ...
    private void createModels(MyHttpRequest httpRequest, boolean add) throws Exception {
        ...
        var m = PCAModel.newModel("pca", fields);
        m.train(tracks);
        if (add) {
            List<Extension> extensions = fields.stream().map(e -> e.extension).distinct().collect(Collectors.toList());
            PCAExtension pcaExtension = new PCAExtension(extensions, m);
            ExtensionManager.INSTANCE.addExtension(pcaExtension);
            MyWebUtils.sendRedirect(httpRequest, getUrl());
        } else {
            MyWebUtils.simpleResponse(httpRequest, "<pre>" + m.pca.toString() + "</pre>");
        }
//        new PCAExtension(fields)

//        ExtensionManager.INSTANCE.addExtension(new PostExtension((Field.ModeField) f)
//        );
//        MyWebUtils.sendRedirect(httpRequest, getUrl());
    }
    ...
    public void appendModelInfoTable(Element element) {
        ...
        for (Extension extension : ExtensionManager.INSTANCE.getAllExtensions()) {
            if (extension instanceof PCAExtension) {
//                element.appendChild("");
